...
def account_auth(user: str, password: str) -> None:
    ...
    try:
        ...
        if any(
            (isinstance(x, str) and x.startswith("(401 unauthorized"))
            for x in excp.args
        ):
    ...
    for key in list(config.keys()):
        if key.startswith("server:"):
            ...
            server = PlexServerConfig(
                name=name, urls=urls, token=config[key]["token"])
    ...
    for server in servers:
        ...
        if server.lasturl:
            try:
                ...
                connection = PlexServer(
                    baseurl=server.lasturl, token=server.token, timeout=5
                )
        ...
        for url in server.urls:
            ...
            try:
                ...
                connection = PlexServer(
                    baseurl=url, token=server.token, timeout=5)
                server.lasturl = url
                server.connection = connection
                break
            except requests.exceptions.ConnectTimeout:
...
def server_config_update(
    config: ConfigParser, servers: List[PlexServerConfig]
) -> ConfigParser:
    """
...
def get_movie_sections(server: PlexServer) -> List[LibrarySection]:
    ...
    """
    movie_sections = [
        section for section in server.library.sections() if section.title == "Movies"
    ]
...
def parse_movies(movie_sections: List[LibrarySection]) -> MovieData:
    ...
    for section in movie_sections:
        ...
        for media in section.all():
            logging.info(
                'Processing(%d/%d) "%s"', movie_count, movie_total, media.title
            )
            movie_count += 1
